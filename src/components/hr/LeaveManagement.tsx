...
import { Calendar, Plus, Clock, CheckCircle, XCircle, Info, ChevronRight, Filter, Download } from 'lucide-react';
...
export default function LeaveManagement() {
  ...
  const fetchData = async () => {
    ...
    try {
      if (isAdmin) {
        ...
        if (res && Array.isArray(res)) setAllLeaves(res);
        else if (res?.data) setAllLeaves(res.data);
      }
      
      const myRes = await apiClient.getMyLeaves();
      console.log('My leaves response:', myRes);
      if (myRes && Array.isArray(myRes)) setMyLeaves(myRes);
      else if (myRes?.data) setMyLeaves(myRes.data);
    } catch (error) {
  ...
  const handleApply = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // API returns the created leave object directly (data.data)
      if (res && res._id) {
      ...
      alert('Failed to apply for leave: ' + (error.message || error));
    }
  };
  ...
  const handleUpdateStatus = async (id: string, status: 'approved' | 'rejected') => {
    ...
    try {
      const res = await apiClient.updateLeaveStatus(id, status, comments || '');
      if (res.success) {
        fetchData();
      }
    } catch (error) {
    ...
    <div className="p-6 max-w-7xl mx-auto font-sans">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">Manage your leave requests and track approvals.</p>
        </div>
      ...
      <div className="bg-white rounded-3xl shadow-xl shadow-gray-100 border border-gray-100 overflow-hidden">
        {loading ? (
          <div className="p-20 text-center">
            ...
            <p className="text-gray-400 font-medium">Loading leave records...</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-50/50 border-b border-gray-100">
                  ...
                  <th className="px-6 py-5 text-xs font-bold text-gray-400 uppercase tracking-widest">Reason</th>
                  ...
                  <tr>
                    <td colSpan={isAdmin ? 6 : 5} className="px-6 py-20 text-center">
                      <div className="max-w-xs mx-auto">
                        ...
                        <p className="text-gray-400 font-bold mb-1">No leave requests found</p>
                        <p className="text-gray-300 text-sm">When you or your team applies for leave, they will appear here.</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={leave?._id} className="hover:bg-gray-50/50 transition-colors group">
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-3">
                          <div className={`p-2.5 rounded-xl ${leave?.leaveType === 'sick' ? 'bg-red-50 text-red-600' : 'bg-blue-50 text-blue-600'}`}>
                            <Calendar size={18} />
                          </div>
                          <span className="font-bold text-gray-700 capitalize">{leave.leaveType}</span>
                        </div>
                      </td>
                      {activeTab === 'all' && (
                        <td className="px-6 py-5">
                          <div className="flex flex-col">
                            <span className="font-bold text-gray-900">{leave.inspector?.firstName} {leave.inspector?.lastName}</span>
                            <span className="text-xs text-gray-400">{leave.inspector?.email}</span>
                          </div>
                        </td>
                      )}
                      <td className="px-6 py-5 text-sm font-medium text-gray-600">
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-blue-600 font-bold mt-1 bg-blue-50 w-fit px-2 py-0.5 rounded-full">{leave.totalDays} Days</span>
                        </div>
                      </td>
                      <td className="px-6 py-5">
                        <p className="text-sm text-gray-600 max-w-xs line-clamp-2">{leave.reason}</p>
                      </td>
                      <td className="px-6 py-5">
                        <span className={`px-4 py-1.5 rounded-full text-xs font-bold border ${getStatusColor(leave?.status || 'pending')}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-[2rem] w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="p-8 bg-[#1e3a8a] text-white">
              ...
              <p className="text-blue-100 opacity-80 font-light">Enter the details of your leave request.</p>
            </div>
            
            <form onSubmit={handleApply} className="p-8 space-y-5">
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Leave Type</label>
                <select 
