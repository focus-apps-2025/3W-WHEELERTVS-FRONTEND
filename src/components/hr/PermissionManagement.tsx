...
import { Clock, Plus, CheckCircle, XCircle, User, Calendar, MapPin, ChevronRight, Filter } from 'lucide-react';
...
export default function PermissionManagement() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (myRes.success) setMyPermissions(myRes.data || []);
    } catch (error) {
  ...
  const handleApply = async (e: React.FormEvent) => {
    ...
    try {
      // Map local time strings to full Date objects for the backend
      const baseDate = formData.date;
      const startDateTime = new Date(`${baseDate}T${formData.startTime}:00`).toISOString();
      const endDateTime = new Date(`${baseDate}T${formData.endTime}:00`).toISOString();

      const res = await apiClient.applyPermission({
        ...formData,
      ...
      alert('Request failed: ' + (error as any).message);
    }
  };

  const handleUpdateStatus = async (id: string, status: 'approved' | 'rejected') => {
    try {
      const res = await apiClient.updatePermissionStatus(id, status);
      if (res.success) {
        fetchData();
      }
    } catch (error) {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto font-sans">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Short Permission</h1>
          <p className="text-gray-500 mt-1">Request short leave, gate pass, or late arrival.</p>
        </div>
      ...
      <div className="bg-white rounded-3xl shadow-xl shadow-gray-100 border border-gray-100 overflow-hidden">
        {loading ? (
          <div className="p-20 text-center">
            ...
            <p className="text-gray-400 font-medium">Loading permission records...</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-50/50 border-b border-gray-100">
                  ...
                  {activeTab === 'all' && <th className="px-6 py-5 text-xs font-bold text-gray-400 uppercase tracking-widest">Employee</th>}
                  <th className="px-6 py-5 text-xs font-bold text-gray-400 uppercase tracking-widest">Timing</th>
                  <th className="px-6 py-5 text-xs font-bold text-gray-400 uppercase tracking-widest">Status</th>
                  {isAdmin && activeTab === 'all' && <th className="px-6 py-5 text-xs font-bold text-gray-400 uppercase tracking-widest">Actions</th>}
                </tr>
              </thead>
                  ...
                  <tr>
                    <td colSpan={isAdmin ? 5 : 4} className="px-6 py-20 text-center">
                      <div className="max-w-xs mx-auto">
                        ...
                        <p className="text-gray-400 font-bold mb-1">No requests found</p>
                        <p className="text-gray-300 text-sm">Permission requests will appear here after submission.</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                  ((activeTab === 'all' ? allPermissions : myPermissions) || []).map((perm) => (
                    <tr key={perm._id} className="hover:bg-gray-50/50 transition-colors group">
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-3">
                          <div className={`p-2.5 rounded-xl ${perm.permissionType === 'short-leave' ? 'bg-purple-50 text-purple-600' : 'bg-amber-50 text-amber-600'}`}>
                            <Clock size={18} />
                          </div>
                          <span className="font-bold text-gray-700 capitalize">{perm.permissionType.replace('-', ' ')}</span>
                        </div>
                      </td>
                      {activeTab === 'all' && (
                        <td className="px-6 py-5">
                          <div className="flex flex-col">
                            <span className="font-bold text-gray-900">{perm.inspector?.firstName} {perm.inspector?.lastName}</span>
                            <span className="text-xs text-gray-400">{perm.inspector?.email}</span>
                          </div>
                        </td>
                      )}
                      <td className="px-6 py-5 text-sm font-medium text-gray-600">
                        <div className="flex flex-col">
                          <span className="font-bold text-gray-700">{format(new Date(perm.date), 'MMM d, yyyy')}</span>
                          <span className="text-xs text-blue-600 font-bold mt-1">
                            {format(new Date(perm.startTime), 'hh:mm a')} - {format(new Date(perm.endTime), 'hh:mm a')} ({perm.duration}h)
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-5">
                        <span className={`px-4 py-1.5 rounded-full text-xs font-bold border ${getStatusColor(perm.status)}`}>
                        ...
                        <td className="px-6 py-5">
                          {perm.status === 'pending' ? (
                            <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button 
                                onClick={() => handleUpdateStatus(perm._id, 'approved')}
                                className="p-2 bg-green-50 text-green-600 rounded-lg hover:bg-green-100 transition-colors"
                              >
                                <CheckCircle size={20} />
                              </button>
                              <button 
                                onClick={() => handleUpdateStatus(perm._id, 'rejected')}
      ...
      {showApplyModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm" onClick={() => setShowApplyModal(false)}></div>
          <div className="relative bg-white rounded-[2rem] w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="p-8 bg-[#1e3a8a] text-white">
              ...
              <p className="text-blue-100 opacity-80 font-light">Short duration permissions for today or upcoming dates.</p>
            </div>
            
            <form onSubmit={handleApply} className="p-8 space-y-5">
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Permission Type</label>
                <select 
