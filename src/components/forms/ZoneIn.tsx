...
const ZoneIn: React.FC<ZoneInProps> = ({
  value,
  onChange,
  onFocus,
  onBlur,
  ...
  const selectedZones = value?.zones || [];
  ...
  const toggleZone = (zone: string) => {
    if (disabled) return;
    const nextZones = selectedZones.includes(zone)
      ? selectedZones.filter((z: string) => z !== zone)
      : [...selectedZones, zone];
    updateValue({ zones: nextZones });
  };
  ...
  const isReworkOrRejected = statusValue === 'Rework' || statusValue === 'Rejected';
        ...
        <div className="animate-in fade-in slide-in-from-top-2 duration-300 space-y-4">
          ...
          {statusValue && (
            <div className="space-y-4 pt-4 border-t border-gray-100 dark:border-gray-800 animate-in fade-in slide-in-from-top-2">
              {/* Zone Selection for Rework/Rejected */}
              {isReworkOrRejected && (
                <div className="space-y-2">
                  <label className="flex items-center gap-1.5 text-[11px] font-bold text-gray-500 uppercase tracking-wider ml-1">
                        ...
                        <button
                          ...
                          className={`px-3 py-2 rounded-lg border text-[10px] font-bold transition-all ${
                            isSelected
                              ...
                              : "bg-white dark:bg-gray-950 text-gray-500 border-gray-200 dark:border-gray-800 hover:border-blue-300"
                          }`}
                        >
                          {zone}
                        </button>
