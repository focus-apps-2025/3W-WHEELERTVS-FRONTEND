...
// Component for Defect Details (Remarks & Evidence)
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              placeholder={`Add details about this defect in ${category}...`}
...
const ChassisWithoutZone: React.FC<ChassisWithoutZoneProps> = ({
  ...
  const handleMainCameraCapture = async (file: File) => {
    try {
  ...
  const getSelectedCategories = (): string[] => {
    return categoriesData.map((cat: DefectCategory) => cat.name);
  };
  ...
  const getSelectedDefectsForCategory = (categoryName: string): string[] => {
    const category = categoriesData.find((cat: DefectCategory) => cat.name === categoryName);
    if (!category) return [];
  ...
  // Get all available defects for a category
  ...
  return (
    ...
    >
      ...
      {/* Step 3: Defect Categories and Defects (No Zone Selection) */}
        ...
        <div className="animate-in fade-in slide-in-from-top-4 duration-500 space-y-4 pt-4 border-t border-gray-100 dark:border-gray-800">
          ...
          {categoriesData.length > 0 && (
            <div className="space-y-4 mt-4">
              {categoriesData.map((category: DefectCategory) => {
                      ...
                      >
                        ...
                        {category.defects.length > 0 && (
                          <span className="text-[10px] font-medium text-purple-600 dark:text-purple-400 bg-purple-100 dark:bg-purple-900/50 px-2 py-0.5 rounded-full">
                            {category.defects.length} defect{category.defects.length !== 1 ? 's' : ''}
                          </span>
                        )}
                      </button>
                    </div>
                      ...
                      <div className="ml-6 space-y-4">
                        ...
                        <div className="overflow-visible">
                          <MultiSelectDropdown
                            ...
                            onChange={(selected) => handleDefectsChange(category.name, selected)}
                            ...
                            {category.defects.map((defect: SpecificDefect) => {
                              ...
                              return (
                                <DefectDetails
                                  ...
                                  onUpdate={(updates) => updateDefectDetails(category.name, defect.name, updates)}
                                  onFileUpload={(file) => handleDefectFileUpload(category.name, defect.name, file)}
                                  disabled={disabled}
