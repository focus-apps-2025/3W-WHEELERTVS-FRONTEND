...
const ChassisWithZone: React.FC<ChassisWithZoneProps> = ({
  ...
  const handleStatusChange = (val: string) => {
    if (val === 'Accepted') {
      updateValue({ status: val, zone: [], defectCategory: "", defects: [] });
    } else {
  ...
  const handleZoneToggle = (zone: string) => {
    if (disabled) return;
    const next = zoneValue.includes(zone)
      ? zoneValue.filter((z: string) => z !== zone)
      : [...zoneValue, zone];
      ...
      {hasChassis && statusValue && !isApplied && (
        <div className="pt-2">
          {isRejectedOrRework ? (
            ...
            <div className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-800/50">
