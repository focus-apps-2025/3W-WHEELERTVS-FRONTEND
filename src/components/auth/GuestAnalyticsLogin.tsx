...
import { Mail, Loader2, BarChart3, ArrowRight, AlertCircle, Phone } from "lucide-react";
...
export default function GuestAnalyticsLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // The backend now directly returns the token, bypassing OTP
      const response = await apiClient.requestAnalyticsOTP(id, email, phone, 'email'); // channel is not used anymore, but let's pass one
      ...
      setError(err.message || "Access denied. Please check your credentials or that you have been invited.");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex flex-col items-center justify-center p-4">
      ...
      <div className="w-full max-w-md relative">
        ...
        <p className="text-center mt-8 text-sm text-gray-500 dark:text-gray-400">
          You must be invited to access this dashboard.
        </p>
      </div>
    </div>
  );
