...
export default function Attendance() {
  ...
  const processAttendanceData = (logs: any[]) => {
    ...
    logs.forEach((log) => {
      ...
      if (loginDate) {
        // HR report returns checkIn/checkOut as strings like "5:10:38 pm"
        // We need to store them as strings, not try to parse as Date
        user.attendance[loginDate] = {
          ...
          isPresent: log.status === "present" || log.status === "late",
    ...
    setUserAttendance(Array.from(userMap.values()));
  };
  ...
  const renderDateCell = (
    ...
    let workingMinutes =
      record.workingMinutes || Math.round(record.workingHours * 60);
    ...
    return (
      <div className="space-y-1 text-xs">
        ...
        <div className="text-gray-500">
          {workingHours > 0 ? `${Math.round(workingHours * 60)}m` : "-"}
        </div>
        {isActive ? (
